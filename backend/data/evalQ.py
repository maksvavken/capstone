...
eval_questions: List[EvalQ] = [
    ...
    EvalQ(
        field="Geometry",
        style="step_by_step",
        q="Find the surface area of a cylinder with radius $r$ and height $h$.",
        gt="Step 1: The two circular caps each have area $\\pi r^2$, so together $2\\pi r^2$. Step 2: Unroll the lateral surface into a rectangle of width $2\\pi r$ and height $h$, giving area $2\\pi r h$. Step 3: Total surface area $= 2\\pi r^2 + 2\\pi r h = 2\\pi r(r + h)$.",
    ),
    EvalQ(
        field="Geometry",
        style="short_simple",
        q="What is the difference between a permutation and a combination?",
        gt="A permutation counts arrangements where order matters (e.g. a lock code), while a combination counts selections where order does not matter (e.g. choosing a team). Permutations are always greater than or equal to combinations for the same $n$ and $k$.",
    ),
    EvalQ(
        field="Geometry",
        style="short_technical",
        q="What is the formula for the distance between two points $(x_1, y_1)$ and $(x_2, y_2)$ in the plane, and why?",
        gt="By the Pythagorean theorem applied to the right triangle with legs $|x_2 - x_1|$ and $|y_2 - y_1|$, the distance is $d = \\sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}$.",
    ),
    EvalQ(
        field="Geometry",
        style="long_simple",
        q="Why is the volume of a sphere $\\frac{4}{3}\\pi r^3$? The $\\frac{4}{3}$ seems random, how could somebody guess this?",
        gt="A sphere equals two cones each with height $r$ and radius $r$. Each cone volume is $\\frac{1}{3}\\pi r^2 h$. With $h = 2r$, each cone is $\\frac{2}{3}\\pi r^3$. Two cones gives $\\frac{2}{3}\\pi r^3 + \\frac{2}{3}\\pi r^3 = \\frac{4}{3}\\pi r^3$.",
    ),
    EvalQ(
        field="Geometry",
        style="long_technical",
        q="Prove that the sum of interior angles of any triangle is $180^\\circ$.",
        gt="Draw line $l$ through vertex $C$ parallel to side $AB$. By the alternate interior angles theorem, $\\angle BAC = \\angle ACL$ and $\\angle ABC = \\angle BCM$ where $L, M$ are points on $l$. Since $\\angle ACL + \\angle ACB + \\angle BCM = 180^\\circ$ (angles on a straight line), substituting gives $\\angle BAC + \\angle ACB + \\angle ABC = 180^\\circ$.",
    ),
    ...
    EvalQ(
        ...
        gt="Step 1: Find two numbers that multiply to $6$ and add to $-5$: those are $-2$ and $-3$. Step 2: Factor: $(x - 2)(x - 3) = 0$. Step 3: Set each factor to zero: $x - 2 = 0 \\Rightarrow x = 2$ and $x - 3 = 0 \\Rightarrow x = 3$. Solutions: $x = 2$ and $x = 3$.",
    ),
    EvalQ(
        field="Algebra",
        style="short_simple",
